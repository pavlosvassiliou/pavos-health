...
LOCK = "/home/pavlos/.lock-pavos-health"
def with_lock(fn):
    fh = open(LOCK, "w")
    for _ in range(10):
...
def _tel(r, label):
    try:
        ...
        model=next(iter(mu)) if mu else d.get("model","?"); p="/home/pavlos/pavos-health/build/logs/telemetry.csv"; new=not os.path.exists(p)
        with open(p,"a") as o:

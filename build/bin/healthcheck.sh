...
B=$(ls -1t build/logs/review-*.txt 2>/dev/null | head -1 | xargs -n1 basename 2>/dev/null); C=$(ls -1t build/logs/review-*.txt 2>/dev/null | head -1 | xargs -n1 basename 2>/dev/null)
